import string
#delta nicht linear Trennbar? => delta = False
#delta linear Trennbar? => delta = True
def create_lp_hyper(function: [str], bounds: [str], x1: [float], y1: [float], x2: [float], y2: [float], delta=T
 
 #Konstanten Initialisierung
 function[0] = ""
 #Generiert die zu maximierende Funktion Funktion
 z = "maximize\n\tdelta"
 s = ""
 #Generiert die Nebenbedingungen anhand der X und Y Werte für Klasse 1
 s += "\n\nsubject to\n"
 ...
 #Generiert die Nebenbedingungen anhand der X und Y Werte für Klasse 2 
 ...
 #Falls die Klassen nicht linear Trennbar sind verändere Lineares Programm
 if not delta:
 z = "minimize\n\t"
 s = s.replace("<=", "=").replace(">=", "=")
 for i in range(len(x1)):
